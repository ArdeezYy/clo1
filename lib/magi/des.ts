const IP = [
  58, 50, 42, 34, 26, 18, 10, 2,
  60, 52, 44, 36, 28, 20, 12, 4,
  62, 54, 46, 38, 30, 22, 14, 6,
  64, 56, 48, 40, 32, 24, 16, 8,
  57, 49, 41, 33, 25, 17, 9, 1,
  59, 51, 43, 35, 27, 19, 11, 3,
  61, 53, 45, 37, 29, 21, 13, 5,
  63, 55, 47, 39, 31, 23, 15, 7,
] as const;

const FP = [
  40, 8, 48, 16, 56, 24, 64, 32,
  39, 7, 47, 15, 55, 23, 63, 31,
  38, 6, 46, 14, 54, 22, 62, 30,
  37, 5, 45, 13, 53, 21, 61, 29,
  36, 4, 44, 12, 52, 20, 60, 28,
  35, 3, 43, 11, 51, 19, 59, 27,
  34, 2, 42, 10, 50, 18, 58, 26,
  33, 1, 41, 9, 49, 17, 57, 25,
] as const;

const E = [
  32, 1, 2, 3, 4, 5,
  4, 5, 6, 7, 8, 9,
  8, 9, 10, 11, 12, 13,
  12, 13, 14, 15, 16, 17,
  16, 17, 18, 19, 20, 21,
  20, 21, 22, 23, 24, 25,
  24, 25, 26, 27, 28, 29,
  28, 29, 30, 31, 32, 1,
] as const;

const P = [
  16, 7, 20, 21,
  29, 12, 28, 17,
  1, 15, 23, 26,
  5, 18, 31, 10,
  2, 8, 24, 14,
  32, 27, 3, 9,
  19, 13, 30, 6,
  22, 11, 4, 25,
] as const;

const PC1 = [
  57, 49, 41, 33, 25, 17, 9,
  1, 58, 50, 42, 34, 26, 18,
  10, 2, 59, 51, 43, 35, 27,
  19, 11, 3, 60, 52, 44, 36,
  63, 55, 47, 39, 31, 23, 15,
  7, 62, 54, 46, 38, 30, 22,
  14, 6, 61, 53, 45, 37, 29,
  21, 13, 5, 28, 20, 12, 4,
] as const;

const PC2 = [
  14, 17, 11, 24, 1, 5,
  3, 28, 15, 6, 21, 10,
  23, 19, 12, 4, 26, 8,
  16, 7, 27, 20, 13, 2,
  41, 52, 31, 37, 47, 55,
  30, 40, 51, 45, 33, 48,
  44, 49, 39, 56, 34, 53,
  46, 42, 50, 36, 29, 32,
] as const;

const SHIFTS = [1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1] as const;

const S_BOXES = [
  [
    [14, 4, 13, 1, 2, 15, 11, 8, 3, 10, 6, 12, 5, 9, 0, 7],
    [0, 15, 7, 4, 14, 2, 13, 1, 10, 6, 12, 11, 9, 5, 3, 8],
    [4, 1, 14, 8, 13, 6, 2, 11, 15, 12, 9, 7, 3, 10, 5, 0],
    [15, 12, 8, 2, 4, 9, 1, 7, 5, 11, 3, 14, 10, 0, 6, 13],
  ],
  [
    [15, 1, 8, 14, 6, 11, 3, 4, 9, 7, 2, 13, 12, 0, 5, 10],
    [3, 13, 4, 7, 15, 2, 8, 14, 12, 0, 1, 10, 6, 9, 11, 5],
    [0, 14, 7, 11, 10, 4, 13, 1, 5, 8, 12, 6, 9, 3, 2, 15],
    [13, 8, 10, 1, 3, 15, 4, 2, 11, 6, 7, 12, 0, 5, 14, 9],
  ],
  [
    [10, 0, 9, 14, 6, 3, 15, 5, 1, 13, 12, 7, 11, 4, 2, 8],
    [13, 7, 0, 9, 3, 4, 6, 10, 2, 8, 5, 14, 12, 11, 15, 1],
    [13, 6, 4, 9, 8, 15, 3, 0, 11, 1, 2, 12, 5, 10, 14, 7],
    [1, 10, 13, 0, 6, 9, 8, 7, 4, 15, 14, 3, 11, 5, 2, 12],
  ],
  [
    [7, 13, 14, 3, 0, 6, 9, 10, 1, 2, 8, 5, 11, 12, 4, 15],
    [13, 8, 11, 5, 6, 15, 0, 3, 4, 7, 2, 12, 1, 10, 14, 9],
    [10, 6, 9, 0, 12, 11, 7, 13, 15, 1, 3, 14, 5, 2, 8, 4],
    [3, 15, 0, 6, 10, 1, 13, 8, 9, 4, 5, 11, 12, 7, 2, 14],
  ],
  [
    [2, 12, 4, 1, 7, 10, 11, 6, 8, 5, 3, 15, 13, 0, 14, 9],
    [14, 11, 2, 12, 4, 7, 13, 1, 5, 0, 15, 10, 3, 9, 8, 6],
    [4, 2, 1, 11, 10, 13, 7, 8, 15, 9, 12, 5, 6, 3, 0, 14],
    [11, 8, 12, 7, 1, 14, 2, 13, 6, 15, 0, 9, 10, 4, 5, 3],
  ],
  [
    [12, 1, 10, 15, 9, 2, 6, 8, 0, 13, 3, 4, 14, 7, 5, 11],
    [10, 15, 4, 2, 7, 12, 9, 5, 6, 1, 13, 14, 0, 11, 3, 8],
    [9, 14, 15, 5, 2, 8, 12, 3, 7, 0, 4, 10, 1, 13, 11, 6],
    [4, 3, 2, 12, 9, 5, 15, 10, 11, 14, 1, 7, 6, 0, 8, 13],
  ],
  [
    [4, 11, 2, 14, 15, 0, 8, 13, 3, 12, 9, 7, 5, 10, 6, 1],
    [13, 0, 11, 7, 4, 9, 1, 10, 14, 3, 5, 12, 2, 15, 8, 6],
    [1, 4, 11, 13, 12, 3, 7, 14, 10, 15, 6, 8, 0, 5, 9, 2],
    [6, 11, 13, 8, 1, 4, 10, 7, 9, 5, 0, 15, 14, 2, 3, 12],
  ],
  [
    [13, 2, 8, 4, 6, 15, 11, 1, 10, 9, 3, 14, 5, 0, 12, 7],
    [1, 15, 13, 8, 10, 3, 7, 4, 12, 5, 6, 11, 0, 14, 9, 2],
    [7, 11, 4, 1, 9, 12, 14, 2, 0, 6, 10, 13, 15, 3, 5, 8],
    [2, 1, 14, 7, 4, 10, 8, 13, 15, 12, 9, 0, 3, 5, 6, 11],
  ],
] as const;

export function desEncryptCbc(data: Uint8Array, keyBytes: Uint8Array, ivBytes: Uint8Array) {
  const roundKeys = createRoundKeys(keyBytes);
  const padded = pkcs7Pad(data, 8);
  const output = new Uint8Array(padded.length);
  let previous = bytesToBigInt(ivBytes);

  for (let offset = 0; offset < padded.length; offset += 8) {
    const block = bytesToBigInt(padded.subarray(offset, offset + 8)) ^ previous;
    const encrypted = desEncryptBlock(block, roundKeys);
    output.set(bigIntToBytes(encrypted, 8), offset);
    previous = encrypted;
  }

  return output;
}

export function desDecryptCbc(data: Uint8Array, keyBytes: Uint8Array, ivBytes: Uint8Array) {
  if (data.length === 0 || data.length % 8 !== 0) {
    throw new Error("Ciphertext DES-CBC harus kelipatan 8 byte.");
  }

  const roundKeys = createRoundKeys(keyBytes);
  const output = new Uint8Array(data.length);
  let previous = bytesToBigInt(ivBytes);

  for (let offset = 0; offset < data.length; offset += 8) {
    const current = bytesToBigInt(data.subarray(offset, offset + 8));
    const decrypted = desDecryptBlock(current, roundKeys) ^ previous;
    output.set(bigIntToBytes(decrypted, 8), offset);
    previous = current;
  }

  return pkcs7Unpad(output, 8);
}

function desEncryptBlock(block: bigint, roundKeys: bigint[]) {
  return processBlock(block, roundKeys);
}

function desDecryptBlock(block: bigint, roundKeys: bigint[]) {
  return processBlock(block, [...roundKeys].reverse());
}

function processBlock(block: bigint, roundKeys: bigint[]) {
  const initial = permute(block, 64, IP);
  let left = Number((initial >> 32n) & 0xffffffffn);
  let right = Number(initial & 0xffffffffn);

  for (const roundKey of roundKeys) {
    const nextLeft = right;
    const nextRight = left ^ feistel(right, roundKey);
    left = nextLeft >>> 0;
    right = nextRight >>> 0;
  }

  const preOutput = (BigInt(right >>> 0) << 32n) | BigInt(left >>> 0);
  return permute(preOutput, 64, FP);
}

function feistel(right: number, roundKey: bigint) {
  const expanded = permute(BigInt(right >>> 0), 32, E);
  const mixed = expanded ^ roundKey;
  let sBoxOutput = 0;

  for (let index = 0; index < 8; index += 1) {
    const shift = BigInt((7 - index) * 6);
    const chunk = Number((mixed >> shift) & 0x3fn);
    const row = ((chunk & 0b100000) >> 4) | (chunk & 0b1);
    const column = (chunk >> 1) & 0b1111;
    sBoxOutput = (sBoxOutput << 4) | S_BOXES[index][row][column];
  }

  return Number(permute(BigInt(sBoxOutput >>> 0), 32, P) & 0xffffffffn);
}

function createRoundKeys(keyBytes: Uint8Array) {
  if (keyBytes.length !== 8) {
    throw new Error("DES key harus 8 byte.");
  }

  const permuted = permute(bytesToBigInt(keyBytes), 64, PC1);
  let left = Number((permuted >> 28n) & 0x0fffffffn);
  let right = Number(permuted & 0x0fffffffn);
  const roundKeys: bigint[] = [];

  for (const shift of SHIFTS) {
    left = rotateLeft28(left, shift);
    right = rotateLeft28(right, shift);
    const combined = (BigInt(left) << 28n) | BigInt(right);
    roundKeys.push(permute(combined, 56, PC2));
  }

  return roundKeys;
}

function rotateLeft28(value: number, shift: number) {
  return ((value << shift) | (value >>> (28 - shift))) & 0x0fffffff;
}

function permute(input: bigint, inputBits: number, table: readonly number[]) {
  let output = 0n;

  for (const position of table) {
    const bit = (input >> BigInt(inputBits - position)) & 1n;
    output = (output << 1n) | bit;
  }

  return output;
}

function pkcs7Pad(data: Uint8Array, blockSize: number) {
  const remainder = data.length % blockSize;
  const padding = remainder === 0 ? blockSize : blockSize - remainder;
  const output = new Uint8Array(data.length + padding);
  output.set(data);
  output.fill(padding, data.length);
  return output;
}

function pkcs7Unpad(data: Uint8Array, blockSize: number) {
  if (data.length === 0 || data.length % blockSize !== 0) {
    throw new Error("Padding DES-CBC tidak valid.");
  }

  const padding = data[data.length - 1];
  if (padding === 0 || padding > blockSize) {
    throw new Error("Padding DES-CBC tidak valid.");
  }

  for (let index = data.length - padding; index < data.length; index += 1) {
    if (data[index] !== padding) {
      throw new Error("Padding DES-CBC tidak valid.");
    }
  }

  return data.subarray(0, data.length - padding);
}

function bytesToBigInt(bytes: Uint8Array) {
  let value = 0n;

  for (const byte of bytes) {
    value = (value << 8n) | BigInt(byte);
  }

  return value;
}

function bigIntToBytes(value: bigint, size: number) {
  const output = new Uint8Array(size);

  for (let index = size - 1; index >= 0; index -= 1) {
    output[index] = Number(value & 0xffn);
    value >>= 8n;
  }

  return output;
}
